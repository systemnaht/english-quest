...
export default async function handler(req,res){
  ...
  const zone=String(p.zoneTitle||'Spawn Island').slice(0,60);
  const pattern=String(p.pattern||'I am / I have / I can').slice(0,90);
  const difficulty=Math.max(0,Math.min(4,Number(p.difficulty)||0));
  const weak=Array.isArray(p.weakPhrases)?p.weakPhrases.slice(0,6):[];
  const mistakes=Array.isArray(p.recentMistakes)?p.recentMistakes.slice(0,5):[];
...
- Use mostly A1 words and very short sentences. A tiny stretch is okay only when context makes it obvious.
...
- No open-ended social chat, no purchases, gambling, loot-box mechanics, real weapons, gore, sexual content, drugs or scary content.
...
{
 "title":"2-5 words",
 "scene":"2 short German sentences setting the mission context",
 "chunks":[{"en":"short useful English chunk","de":"German meaning"},{"en":"...","de":"..."},{"en":"...","de":"..."}],
 "exercises":[
   {"type":"meaning|listen|build|type|response","prompt":"...","context":"short German hint/context","best":"best English answer","answerText":"exact accepted answer or meaning","audio":"English sentence for listen","options":["..."],"pieces":["..."] ,"answerIndex":0}
 ]
}
Rules: exactly 3 chunks and exactly 6 exercises. Mix at least 4 exercise types. For meaning/listen/response use exactly 4 options. For response, the correct option MUST be first and answerIndex=0. For build, pieces must reconstruct best. For type, answerText should equal best. For meaning, answerText is the correct German meaning. For listen, answerText is the exact English audio sentence. Keep every prompt under 18 words.`;
  ...
  try{
    ...
    if(!r.ok)return res.status(r.status).json({error:'gateway',message:raw.slice(0,900)});
    const out=JSON.parse(raw);
    const content=out.choices?.[0]?.message?.content||'{}';
    const clean=content.replace(/^```json\s*/i,'').replace(/^```\s*/,'').replace(/```$/,'').trim();
    ...
    const exercises=j.exercises.slice(0,6).map(e=>({
      ...
      prompt:String(e.prompt||'').slice(0,180),
      context:String(e.context||'').slice(0,220),
      best:String(e.best||'').slice(0,160),
      answerText:String(e.answerText||e.best||'').slice(0,160),
      audio:String(e.audio||e.best||'').slice(0,160),
      options:Array.isArray(e.options)?e.options.slice(0,4).map(x=>String(x).slice(0,120)):[],
      pieces:Array.isArray(e.pieces)?e.pieces.slice(0,10).map(x=>String(x).slice(0,60)):[],
      answerIndex:Number.isInteger(e.answerIndex)?e.answerIndex:0
    }));
    ...
    return res.status(500).json({error:'generation',message:String(e.message||e).slice(0,500)});
  }
}
