...
export default async function handler(req,res){
  ...
  const b=req.body||{}, level=b.profile?.estimatedLevel||b.profile?.level||b.level||'B1+', weak=b.weakTenses||b.weak||[], length=Math.max(5,Math.min(10,Number(b.length)||8));
  const prompt=`Create ${length} adaptive everyday-English exercises for a German-speaking learner at ${level}. Weak areas: ${weak.join(', ')||'mixed grammar'}. Return only JSON: {"exercises":[...]}. Each item: type mcq|translate|listen, tense, prompt, options, answer, best, accepted, explain, audio. MCQ has exactly 4 options and answer 0-3; translate/listen options=[], answer=-1. Natural spoken English, mixed tenses, useful chunks, concise explanations, no markdown.`;
  try{
